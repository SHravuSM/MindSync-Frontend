...
// export default function FeedPlus() {
//   const { pos } = useParams(); // pos is the postId
...
//   // Fetch post data using pos parameter
...
//     e.preventDefault();
//     if (!newComment.trim() || isCommenting || !pos) return;
...
//     try {
//       const res = await api.post(`user/posts/suggestions/${pos}`, commentData);
...
//     try {
//       const res = await api.get(`/user/posts/suggestions/${pos}`);
...
//   useEffect(() => {
//     if (open === true && pos && !suggestions.length) {
//       fetchSuggestions();
//     }
//   }, [open, fetchSuggestions, pos, suggestions.length]);

//   const collabUsers = async () => {
//     if (!pos) return;

//     try {
//       const res = await api.get(`/posts/collab-users/${pos}`);
...
//     if (isLiking || !pos) return; // Prevent double clicks

//     setIsLiking(true);
//     try {
//       const res = await api.patch(`/user/posts/like/${pos}`, {
//         userId,
//       });
//       setPost(res.data);
//     } catch (err) {
...
//   const onNotImpressed = async () => {
//     if (isCollabbing || !pos) return; // Prevent double clicks

//     setIsCollabbing(true);
//     try {
//       const res = await api.patch(`/user/posts/impression/${pos}`, {
//         userId,
//       });
//       setPost(res.data);
//     } catch (err) {
...
//   if (isLoadingPost) {
//     return (
//       <div className="lg:w-xl w-full space-y-1 flex items-center justify-center py-12">
...
//           <Link to={`/${pos}`} className="relative overflow-hidden">
//             <div
//               ref={contentRef}
...
//                 {Array(
//                   Post.impressions.length < 4 ? Post.impressions.length : 2
//                 )
//                   .fill(0)
//                   .map((e, idx) => (
//                     <div
...
//   // Extract post from the response data
...
//     console.log("Fetching post with ID:", pos);
//     setIsLoadingPost(true);
//     try {
//       const res = await api.get(`/user/posts/${pos}`);
//       console.log("Fetched post response:", res.data);
...
//   const handleCommentSubmit = async (e) => {
//     e.preventDefault();
//     if (!newComment.trim() || isCommenting || !pos || !userId) {
...
//       if (res.data) {
//         // If the response contains the full post data
//         if (res.data.post) {
//           setPostData(res.data);
//         }

//         // Update suggestions regardless
//         if (res.data.suggestions) {
//           setSuggestions(res.data.suggestions);
...
//         if (res.data.post?.commentsCount) {
//           setPostData((prev) => ({
//             ...prev,
//             post: {
//               ...prev.post,
...
//   // FIXED: Like functionality with better error handling
//   const handleLike = async () => {
...
//   // FIXED: Collab functionality with better error handling
//   const onNotImpressed = async () => {
//     if (isCollabbing || !pos || !userId) {
//       console.log("Collab action blocked:", {
//         isCollabbing,
//         hasPos: !!pos,
//         hasUserId: !!userId,
//       });
...
//     console.log("Handling collab for post:", pos, "by user:", userId);
//     setIsCollabbing(true);
...
//   // FIXED: Fetch suggestions with better error handling
//   const fetchSuggestions = useCallback(async () => {
//     if (!pos) return;

//     console.log("Fetching suggestions for post:", pos);
//     setIsLoadingComments(true);
//     try {
//       const res = await api.get(`/user/posts/suggestions/${pos}`);
//       console.log("Suggestions response:", res.data);
//       setSuggestions(res.data.suggestions || res.data.suggestion || []);
//     } catch (err) {
//       console.error("Failed to fetch suggestions:", err);
//       console.error("Error response:", err.response?.data);
//       setSuggestions([]);
...
//   useEffect(() => {
//     if (open === true && pos && postData && suggestions.length === 0) {
//       fetchSuggestions();
//     }
//   }, [open, fetchSuggestions, pos, postData, suggestions.length]);
...
//     metadata.isLiked ||
//     Post?.likes?.includes(userId) ||
//     likedBy.some((user) => user._id === userId);
...
//         <div className="bg-gray-100 dark:bg-gray-800 p-2 text-xs rounded mb-4">
//           <p>Debug Info:</p>
//           <p>Post ID: {pos}</p>
//           <p>User ID: {userId}</p>
//           <p>Post exists: {!!Post}</p>
...
//           onDoubleClick={(e) => {
//             e.stopPropagation();
//             console.log("Double click - attempting to like");
//             handleLike();
//           }}
//           className={`relative ${
//             !dark
//               ? "bg-white/95 text-black lg:border-gray-100 border-gray-300"
//               : "bg-black/95 text-white border-gray-800"
//           } backdrop-blur-xl p-4 lg:pl-5 pt-3 pb-4 rounded-sm border-[0.1px] transition-all duration-700 ease-out cursor-pointer overflow-hidden group`}
//           draggable="true"
//         >
//           {/* Rest of your JSX remains the same, just adding onClick handlers with logging */}

//           {/* Header */}
//           <div className="flex flex-col gap-3 z-10 relative mb-4">
//             <div className="flex items-center justify-between">
//               {author && (
//                 <div className="flex items-center gap-3">
//                   <img
//                     src={author.photo || USER}
//                     alt={author.name}
...
//             <div className="flex items-center justify-between">
//               <h1
//                 className={`text-xl font-bold flex-1 transition-all duration-500 ${
//                   isHovering
//                     ? "transform translate-x-1 text-blue-600 dark:text-blue-400"
...
//           {/* Content */}
//           <div className="relative overflow-hidden block mb-4">
//             <div
//               ref={contentRef}
...
//                 <span
//                   key={idx}
//                   className={`px-3 py-1.5 text-sm rounded-full transition-all duration-500 hover:scale-110 cursor-pointer transform ${
//                     dark
//                       ? "bg-gradient-to-r from-gray-800/80 to-gray-700/80 text-gray-300 hover:from-gray-700 hover:to-gray-600 shadow-lg border border-gray-600/30"
//                       : "bg-gradient-to-r from-gray-100/80 to-gray-200/80 text-gray-700 hover:from-gray-200 hover:to-gray-300 shadow-md border border-gray-300/30"
//                   } hover:shadow-xl hover:-translate-y-0.5 backdrop-blur-sm`}
...
//           {/* Action buttons */}
//           <div className="flex items-center justify-between text-sm z-10 relative border-t border-gray-200/50 dark:border-gray-700/50 pt-4">
//             <div className="flex lg:gap-6 gap-4 items-center text-sm">
//               {/* Comments button */}
//               <button
//                 className={`flex items-center gap-2 cursor-pointer transition-all duration-300 hover:scale-110 group ${
//                   !dark ? "text-gray-700" : "text-gray-300"
//                 } ${open ? "text-blue-500 scale-110" : "hover:text-blue-500"}`}
//                 onClick={(e) => {
//                   e.stopPropagation();
...
//                 />
//                 <span className="font-medium text-sm">
//                   {Post.commentsCount || suggestions.length || 0}
//                 </span>
//               </button>
...
//                   stroke={isLiked ? "red" : !dark ? "#374151" : "#D1D5DB"}
//                   strokeWidth="2"
//                   strokeLinecap="round"
//                   strokeLinejoin="round"
//                 >
...
//                 }}
//                 disabled={isCollabbing}
//                 className={`text-sm font-medium rounded-full px-4 py-2 transition-all duration-300 hover:scale-110 active:scale-95 transform hover:shadow-lg disabled:opacity-50 bg-blue-500 text-white border border-blue-500 hover:bg-blue-600`}
//               >
//                 {isCollabbing ? "Processing..." : "Collab"}
//               </button>
//             </div>
//           </div>
...
//               className={`mt-4 ${
//                 dark ? "text-white" : "text-black"
//               } relative z-10 max-w-2xl transition-all duration-700 ease-out border-t border-gray-200/50 dark:border-gray-700/50 pt-4`}
//             >
//               <div className="flex items-center gap-2 mb-4">
//                 <h3 className="text-base font-semibold text-blue-500 flex items-center gap-2">
...
//               <form
//                 onSubmit={handleCommentSubmit}
//                 className={`mb-6 sticky top-0 z-10 pb-3 ${
//                   dark ? "bg-black/95" : "bg-white/95"
//                 } backdrop-blur-sm`}
//               >
//                 <div className="flex gap-3">
...
//                       onChange={(e) => setNewComment(e.target.value)}
//                       placeholder="Add a suggestion..."
//                       className={`w-full px-0 py-3 text-sm bg-transparent border-0 border-b-2 transition-all duration-200 focus:outline-none ${
//                         dark
//                           ? "border-gray-700 text-white placeholder-gray-500 focus:border-blue-400"
//                           : "border-gray-200 text-black placeholder-gray-400 focus:border-blue-400"
//                       }`}
...
//                         type="submit"
//                         disabled={!newComment.trim() || isCommenting}
//                         className="text-sm px-4 py-2 bg-blue-500 text-white rounded-full hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 hover:scale-105 active:scale-95 font-medium"
//                       >
//                         {isCommenting ? "Posting..." : "Post suggestion"}
//                       </button>
//                       <span
//                         className={`text-xs ${
//                           dark ? "text-gray-400" : "text-gray-500"
