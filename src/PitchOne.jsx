...
const PitchOne = () => {
  ...
  // Extract and calculate metrics - FIXED field access
  const financials = pitch.financials || {};
  const productMetrics = pitch.productMetrics || {};
  const market = pitch.market || {};
  const fundingDetails = pitch.fundingDetails || {};
  ...
  // FIXED: Financial calculations with proper fallbacks
  const monthlyRevenue =
    financials.monthlyRevenue || financials.monthlyRecurringRevenue || 0;
  const annualRevenue =
    financials.annualRevenue ||
    financials.annualRecurringRevenue ||
    monthlyRevenue * 12;
  const burnRate = financials.burnRate || 0;
  const runwayMonths = financials.runwayMonths || 0;
  const customerAcquisitionCost = financials.customerAcquisitionCost || 0;
  const lifetimeValue = financials.lifetimeValue || 0;
  const grossMargin =
    financials.grossMarginPercent || financials.grossMargin || 0;
  const netMargin = financials.netMargin || 0;
  const ebitda = financials.EBITDA || financials.ebitda || 0;
  const yearOverYearGrowth = financials.yearOverYearGrowthPercent || 0;
  const monthOverMonthGrowth = financials.monthOverMonthGrowthPercent || 0;
  ...
  // FIXED: Funding details extraction
  const askAmount =
    fundingDetails.askAmount ||
    fundingDetails.fundingAskAmount ||
    pitch.fundingAsk ||
    0;
  const equityOffered =
    fundingDetails.equityOffered ||
    fundingDetails.equityOfferedPercent ||
    pitch.equityOffered ||
    0;

  // FIXED: Calculate implied valuation if not provided
  const calculateImpliedValuation = (ask, equity) => {
    if (ask > 0 && equity > 0) {
      return (ask / equity) * 100; // Convert percentage to actual valuation
    }
    return 0;
  };
  ...
  const valuation =
    ...
    financials.preMoneyValuation ||
    fundingDetails.impliedValuation ||
    calculateImpliedValuation(askAmount, equityOffered);
  ...
  // FIXED: Team metrics calculation
  const foundingTeamSize = pitch.foundingTeam?.length || 0;
  const teamMembersSize = pitch.teamMembers?.length || 0;
  const teamSize =
    pitch.teamStrength || foundingTeamSize + teamMembersSize || 1;
  const advisorsCount = pitch.advisors?.length || 0;
  ...
  const financialHealth =
    (grossMargin > 70 ? 30 : grossMargin > 40 ? 20 : 10) +
    (monthlyRevenue > 0 ? 25 : 0) +
    (runwayMonths > 18
      ? 25
      : runwayMonths > 12
  ...
  // FIXED: Format helpers with null safety
  const formatCurrency = (amount) => {
  ...
  const formatMultiple = (value) => `${(value || 0).toFixed(1)}x`;
      ...
      >
        ...
        <header className="lg:px-8 px-4 lg:py-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex justify-between items-start mb-6">
            ...
            {/* FIXED: Header funding display */}
            <div className="text-right">
              <div className="text-3xl font-light text-emerald-600 mb-1">
                {formatCurrency(askAmount)}
              </div>
              <div className="text-sm text-gray-500 mb-2">
                for {equityOffered}% equity
              </div>
              <div className="text-xs text-gray-400">
                Implied Valuation: {formatCurrency(valuation)}
              </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                >
                  {yearOverYearGrowth > 0 ? "+" : ""}
                  {formatPercent(yearOverYearGrowth)} YoY
                </span>
              </div>
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <span
                  className={`text-xs ${
                    retentionRate > 80 ? "text-green-600" : "text-gray-500"
                  }`}
                >
                  {formatPercent(retentionRate)} retention
                </span>
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <Clock
                  size={20}
                  className={
                    runwayMonths < 12 ? "text-red-600" : "text-green-600"
                  }
                />
                <span
                  className={`text-xs ${
                  ...
                  >
                    ...
                    <div className="space-y-2 text-sm">
                      <div>
                        ...
                        {pitch.targetMarket || "Not specified"}
                      </div>
                      <div>
                        <strong>Business Model:</strong>{" "}
                        {pitch.businessModel || "Not specified"}
                      </div>
                      {pitch.distributionChannels &&
                        pitch.distributionChannels.length > 0 && (
                          <div>
                            <strong>Distribution:</strong>{" "}
                            {pitch.distributionChannels.join(", ")}
                          </div>
                        )}
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between">
                        <span className="text-sm">Last Year</span>
                        <span className="font-medium">
                          {formatCurrency(financials.revenueLastYear || 0)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm">This Year</span>
                        <span className="font-medium">
                          {formatCurrency(financials.revenueThisYear || 0)}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between">
                        <span className="text-sm">Sales Growth</span>
                        <span className="font-medium">
                          {formatPercent(financials.salesGrowthYoY || 0)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm">Market Growth</span>
                        <span className="font-medium">
                          {formatPercent(marketGrowthRate)}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-medium">
                          {formatCurrency(customerAcquisitionCost)}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm">Lifetime Value</span>
                  ...
                  >
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        ...
                        <span
                          className={`font-medium ${
                            burnRate > 0 ? "text-red-600" : "text-green-600"
                          }`}
                        >
                          {formatCurrency(burnRate)}
                        </span>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-medium">
                          {formatCurrency(
                            financials.cashFlowFromOperations || 0
                          )}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm">Total Assets</span>
                        <span className="font-medium">
                          {formatCurrency(financials.assetsTotal || 0)}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm">Total Liabilities</span>
                        <span className="font-medium">
                          {formatCurrency(financials.liabilitiesTotal || 0)}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm">Debt to Equity</span>
                        <span className="font-medium">
                          {formatMultiple(financials.debtEquityRatio || 0)}
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                  ...
                  >
                    ...
                    <div className="text-sm text-gray-500">
                      SAM - Realistic market reach
                    </div>
                    <div className="mt-4 text-xs">
                      <div className="flex justify-between">
                        <span>% of TAM</span>
                        <span className="font-medium">
                          {tam > 0 ? formatPercent((sam / tam) * 100) : "0%"}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div className="text-sm text-gray-500">
                      SOM - Immediate target market
                    </div>
                    <div className="mt-4 text-xs">
                      <div className="flex justify-between">
                        <span>% of SAM</span>
                        <span className="font-medium">
                          {sam > 0 ? formatPercent((som / sam) * 100) : "0%"}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                {pitch.customerSegments &&
                  pitch.customerSegments.length > 0 && (
                    <div
                      className={`p-6 rounded-lg ${
                        dark ? "bg-gray-700" : "bg-gray-50"
                      }`}
                          ...
                          >
                            <div className="flex justify-between items-start mb-2">
                              ...
                              <span className="text-sm font-bold">
                                {segment.size?.toLocaleString() || 0} users
                              </span>
                            </div>
                            <p className="text-sm text-gray-500">
                              {segment.painPoints || "No pain points specified"}
                            </p>
                          </div>
                        ))}
                      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  >
                    ...
                    <p className="text-sm mb-4">
                      {pitch.competition || "No competitive analysis provided"}
                    </p>
                    <div className="text-sm">
                      <strong>Competitive Advantage:</strong>
                      <p className="mt-2">
                        {pitch.competitiveAdvantage || "Not specified"}
                      </p>
                    </div>
                  </div>
                            ...
                            >
                              ...
                              <div className="text-xs space-y-1">
                                <div>
                                  ...
                                  {competitor.strength || "Not specified"}
                                </div>
                                <div>
                                  <strong>Weakness:</strong>{" "}
                                  {competitor.weakness || "Not specified"}
                                </div>
                              </div>
                            </div>
                          ))}
          ...
          {activeTab === "traction" && (
            <div className="space-y-8">
              ...
              <section>
                  ...
                  >
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        <span className="text-sm">Total Users</span>
                        <span className="font-medium">
                          {pitch.usersCount?.toLocaleString() || 0}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm">Monthly Active</span>
                      ...
                      <div className="flex justify-between">
                        <span className="text-sm">DAU/MAU Ratio</span>
                        <span className="font-medium">
                          {mau > 0 ? formatPercent((dau / mau) * 100) : "0%"}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between">
                        <span className="text-sm">Downloads</span>
                        <span className="font-medium">
                          {productMetrics.downloads?.toLocaleString() || 0}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm">GMV</span>
                        <span className="font-medium">
                          {formatCurrency(productMetrics.GMV || 0)}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        <span className="text-sm">Gross Order Value</span>
                        <span className="font-medium">
                          {formatCurrency(financials.grossOrderValue || 0)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm">Inventory Turnover</span>
                        <span className="font-medium">
                          {formatMultiple(financials.inventoryTurnover || 0)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm">Current Ratio</span>
                        <span
                          className={`font-medium ${
                            (financials.currentRatio || 0) > 2
                              ? "text-green-600"
                              : (financials.currentRatio || 0) > 1
                              ? "text-yellow-600"
                              : "text-red-600"
                          }`}
                        >
                      ...
                      <div className="flex justify-between">
                        <span className="text-sm">ROI</span>
                        <span className="font-medium">
                          {formatPercent(financials.roi || 0)}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                {/* Key Achievements */}
                {productMetrics.keyAchievements &&
                  productMetrics.keyAchievements.length > 0 && (
                    <div
                      className={`p-6 rounded-lg ${
                        dark ? "bg-gray-700" : "bg-gray-50"
                      }`}
                    >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {productMetrics.keyAchievements.map(
                          (achievement, index) => (
                            <div
                              key={index}
                ...
                {/* Customer Testimonials */}
                {pitch.customerTestimonials &&
                  pitch.customerTestimonials.length > 0 && (
                    <div
                      className={`p-6 rounded-lg ${
                        dark ? "bg-gray-700" : "bg-gray-50"
                      }`}
                    >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {pitch.customerTestimonials.map(
                          (testimonial, index) => (
                            <div
                              key={index}
                              className={`p-4 rounded ${
            ...
            <div className="space-y-8">
              ...
              <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                  ...
                  >
                    <div className="text-3xl font-bold mb-2">
                      {pitch.boardOfDirectors?.length || 0}
                    </div>
                    <div className="text-sm text-gray-500">Board Members</div>
                  </div>
                </div>
                      ...
                      {pitch.foundingTeam.map((member, index) => (
                        ...
                        >
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded">
                              {member.experienceYears || 0} yrs exp
                            </span>
                          </div>
                          <p className="text-sm">{member.background}</p>
                          {member.linkedIn && (
                  ...
                  >
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span
                          className={
                            teamSize > 10 ? "text-green-600" : "text-yellow-600"
                          }
                        >
                          {teamSize > 10 ? "Low" : "Medium"}
                        </span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <div className="space-y-2">
                        ...
                        {ltv2cacRatio < 1 && (
                          <div className="flex items-center text-red-600 text-sm">
                            <AlertTriangle size={14} className="mr-2" />
                            Poor unit economics (LTV:CAC ={" "}
                            {formatMultiple(ltv2cacRatio)})
                          </div>
                        )}
                        {grossMargin < 40 && (
                          <div className="flex items-center text-yellow-600 text-sm">
                        ...
                        {runwayMonths >= 6 &&
                          ...
                          churnRate <= 10 && (
                            <div className="flex items-center text-green-600 text-sm">
                              ...
                              No critical financial risks identified
                            </div>
                          )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        ...
                        {legal.complianceStatus !== "Compliant" && (
                          <div className="flex items-center text-red-600 text-sm">
                            <AlertTriangle size={14} className="mr-2" />
                            Compliance issues
                          </div>
                  ...
                  >
                    ...
                    <div className="space-y-3 text-sm">
                      <div className="flex justify-between">
                        <span>Company Type</span>
                        <span className="font-medium">
                          {legal.companyRegistration || "Not specified"}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>ESOP Pool</span>
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              >
                {pitch.founderEmail || "No email provided"}
              </div>
            </div>
            <div className="flex items-center space-x-4">
              {pitch.founderEmail && (
