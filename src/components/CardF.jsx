...
//   // Enhanced animation states - CHANGED: Content now defaults to expanded
...
//     const res = await api.get(`/user/posts/author/${post.authorId}`);
//     setAuthor(res.data);
//   }
...
//   // Improved comment submission with proper state management
//   const handleCommentSubmit = async (e) => {
//     e.preventDefault();
//     if (!newComment.trim() || isCommenting) return;
...
//       // Update both suggestions and post state properly
//       if (res.data.suggestions) {
//         setSuggestions(res.data.suggestions);
//       }
//       if (res.data.post) {
...
//   // Improved fetchSuggestions with better error handling
//   const fetchSuggestions = useCallback(async () => {
//     if (!Post?._id) return;
...
//   // Only fetch when opening comments section
//   useEffect(() => {
//     if (open === true && Post?._id && !suggestions.length) {
//       fetchSuggestions();
//     }
...
//   // Initialize post state properly
//   useEffect(() => {
//     if (post) {
//       setPost(post);
//       // Reset suggestions when post changes
...
//         {/* Header with enhanced animations */}
//         <div className="flex items-center w-full justify-between z-10 relative py-2 pb-1">
//           <h1
//             className={`text-lg font-light text-ellipsis whitespace-nowrap flex-1 mr-4 transition-all duration-500 transform text-white dark:text-black"
//                 : ""
//             }`}
//           >
//             {author}
...
//         {/* Content Section - Full view by default */}
//         <h1
...
//           }`}
//         >
//           {/* Content now has full opacity and is expanded by default */}
//           <p
//             className={`text-sm leading-relaxed whitespace-pre-wrap transition-colors duration-300 ${
//               !isContentExpanded && showExpandButton ? "line-clamp-5" : ""
//             } ${dark ? "text-gray-100" : "text-gray-800"}`}
...
//             {/* Enhanced Reading Time - Shows when expanded */}
//             {showExpandButton && isContentExpanded && (
//               <div
//                 className={`flex items-center justify-end text-xs transition-all duration-500 ${
//                   dark ? "text-gray-400" : "text-gray-500"
...
//                   title={isContentExpanded ? "Show less" : "Read more"}
//                   aria-label={isContentExpanded ? "Show less" : "Read more"}
//                   className={`flex items-center justify-center w-8 h-8 rounded-sm transition-all duration-300 hover:shadow-md backdrop-blur-sm hover:scale-110`}
//                   style={{ minWidth: "2rem", minHeight: "2rem" }}
//                   type="button"
//                 >
//                   {isContentExpanded ? (
//                     <ChevronUp className="w-4 h-4 transition-transform hover:scale-110" />
//                   ) : (
//                     <ChevronDown className="w-4 h-4 transition-transform hover:scale-110" />
//                   )}
//                   <span className="sr-only">
//                     {isContentExpanded ? "Show less" : "Read more"}
//                   </span>
...
//               }}
//               disabled={isLiking}
//               className={`flex items-center gap-1 cursor-pointer transition-all duration-300 hover:scale-110 ${
//                 !dark ? "text-gray-700" : "text-gray-300"
//               } ${
//                 Post.likes?.includes(userId)
//                   ? "text-red-500"
...
const CardF = ({ post }) => {
  ...
  // Fixed fetchSuggestions with better error handling
  const fetchSuggestions = useCallback(async () => {
    if (!Post?._id) return;

    setIsLoadingComments(true);
    try {
      const id = Post._id; // Fixed: Use Post instead of post
      const res = await api.get(`/user/posts/suggestions/${id}`);
      // Handle different response structures
      setSuggestions(res.data.suggestion || res.data.suggestions || []);
    } catch (err) {
  ...
  // Fixed: Added proper dependencies and removed infinite loop
  useEffect(() => {
    if (post) {
      fetchAuthor();
    }
  }, [post?.authorId, fetchAuthor]);
  ...
  // Fixed: Close options when clicking outside with proper cleanup
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (showOptions && !event.target.closest(".options-menu")) {
        setShowOptions(false);
  ...
  // Fixed: CSS custom properties for scrollbar
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Fixed: Scrollable comments list with proper styles */}
          <div
            className={`space-y-4 overflow-y-auto scrollbar-thin ${
              suggestions.length > 3 ? "max-h-64" : "max-h-fit"
            } pr-2`}
            ...
            {isLoadingComments ? (
              ...
              <>
                {suggestions.map((comment, index) => (
                  <div
                    key={comment._id || index}
                    className={`flex gap-3 animate-in slide-in-from-left duration-300 py-2 border-b last:border-b-0 transition-all hover:bg-opacity-50 hover:px-2 hover:-mx-2 ${
                      dark
                        ? "border-gray-800 hover:bg-gray-800/30"
                        : "border-gray-100 hover:bg-gray-50/50"
                    }`}
      ...
      {/* Fixed: Enhanced custom styles with proper CSS */}
      <style jsx>{`
        .scrollbar-thin::-webkit-scrollbar {
