...
//   return (
//     <div className={`max-w-2xl mx-auto px-4 py-6 ${dark ? 'text-black' : 'text-white'}`}>
//       {/* Composer */}
...
const Post = () => {
  ...
  const user = {
    name: "Shravan",
    email: "shravan@example.com",
    photo: "https://i.pravatar.cc/100?u=shravan",
  };
  ...
  return (
    <div className={`max-w-2xl mx-auto px-4 py-6 ${dark ? 'text-black' : 'text-white'}`}>
      <form
        ...
        className={`rounded-md p-6 space-y-4 border ${dark ? 'border-gray-300 bg-white' : 'border-gray-700 bg-[#1f1f1f]'}`}
      >
        <h2 className="text-xl font-semibold">Share a new Idea</h2>
        ...
        <input
          ...
          maxLength={100}
          required
          className={`w-full px-4 py-2 rounded-md text-base border focus:outline-none focus:ring-2 ${dark
        ...
        <textarea
          ...
          placeholder="What's your idea?"
          required
          className={`w-full px-4 py-3 rounded-md resize-y text-base border focus:outline-none focus:ring-2 ${dark
            ? 'bg-white border-gray-300 text-black focus:ring-black'
            : 'bg-[#2a2a2a] border-gray-600 text-white focus:ring-white'
            }`}
        />
        ...
        <div>
          <div className="flex gap-2">
            <input
              ...
              onChange={(e) => setTagInput(e.target.value)}
              placeholder="Add a tag (e.g. tech)"
              className={`flex-grow px-4 py-2 rounded-md text-sm border focus:outline-none focus:ring-2 ${dark
                ? 'bg-white border-gray-300 text-black focus:ring-black'
                : 'bg-[#2a2a2a] border-gray-600 text-white focus:ring-white'
                }`}
            />
            <button
              onClick={handleTagAdd}
              className={`px-4 py-2 text-sm font-medium rounded-md transition ${dark ? 'bg-black text-white hover:bg-gray-800' : 'bg-white text-black hover:bg-gray-100'
                }`}
              disabled={!tagInput.trim()}
            >
              Add
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {tags.map((tag, idx) => (
              <span
                key={idx}
                className={`px-3 py-1 text-sm rounded-full flex items-center gap-2 ${dark ? 'bg-gray-200 text-black' : 'bg-gray-700 text-white'
                  }`}
              >
                #{tag}
                <button
                  type="button"
                  onClick={() => handleTagRemove(tag)}
                  className={`${dark ? 'text-gray-600 hover:text-red-500' : 'text-gray-300 hover:text-red-400'}`}
                >
                  ×
                </button>
              </span>
        ...
        <div className="text-right">
          <button
            type="submit"
            className={`px-6 py-2 font-medium rounded-md transition ${dark ? 'bg-black text-white hover:bg-gray-800' : 'bg-white text-black hover:bg-gray-100'
              }`}
            disabled={!title.trim() || !content.trim()}
          >
            {!loading ? 'Post' : <PostSubmitLoader />}
